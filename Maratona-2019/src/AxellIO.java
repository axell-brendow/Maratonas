...
public class AxellIO
{
    ...
    {
        boolean success = ( AloneArray.exists(printStream) );
    ...
    public static boolean setPrintStream(OutputStream outputStream)
    {
        ...
        if (AloneArray.exists(outputStream))
        {
            try
            {
    ...
    {
        return
        	( AloneArray.exists(msg) ? msg : "Erro" ) +
        	" (fileName = " +
        	( AloneArray.exists(fileName) ? fileName : "unknown" ) + ")";
    }
    ...
    {
        if (AloneArray.exists(myClass, errorMessage, methodReference))
        {
    ...
    {
        if (AloneArray.exists(printStream))
        {
            printStream.close();
        }
    }
    ...
    public static boolean closeInputStream(InputStream inputStream)
    {
        ...
        if (AloneArray.exists(inputStream))
        {
            try
            {
                inputStream.close();
    ...
    public static FileLock lockFileOutputStream(FileOutputStream fileOutputStream)
    {
        ...
        if (AloneArray.exists(fileOutputStream))
        {
            try
            {
    ...
    public static boolean copyChildsOf(File sourceFolder, File destinationFolder, CopyOption... copyOptions)
    {
        ...
        if
        (
            AloneArray.exists(sourceFolder, destinationFolder) &&
    ...
    public static boolean copyChildsOf(String sourceFolderStr, String destinationFolderStr, CopyOption... copyOptions)
    {
        ...
        if (AloneArray.exists(sourceFolderStr, destinationFolderStr))
        {
            success =
            copyChildsOf
            (
    ...
    public static boolean copyFilesRecursively(File sourceFolder, File destinationFolder, CopyOption... copyOptions)
    {
        ...
        if (AloneArray.exists(sourceFolder, destinationFolder))
        {
            success = true;
    ...
    public static String readSpecificString(String text, char[] accept)
    {
        ...
        boolean isAcceptingNumber = AloneArray.contains('d', accept) || AloneArray.contains('f', accept);
        char input;
        ...
        } while (isPossibleToRead() && ( AloneMyString.newLineOnInput(input) || input == ' ' ));

        if (isPossibleToRead() && ( AloneMyString.isOnInterval(input, accept) || (isAcceptingNumber && input == '-') ))
        {
            specificString = "" + input;

            input = read();

            while (isPossibleToRead() && ( AloneMyString.isOnInterval(input, accept) ))
            {
                specificString += input;
                input = read();
            }

            if (!isPossibleToRead() || (!AloneMyString.newLineOnInput(input) && input != ' ' && input != ',') || (isAcceptingNumber && specificString.equals("-")))
            {
                specificString = null;
            }
        }
    ...
    public static String readLine()
    {
        ...
        while (isPossibleToRead() && !AloneMyString.newLineOnInput(input))
        {
            line += input;
            input = read();
        }
    ...
    public static String readLineS()
    {
        ...
        while (AxellIO.isPossibleToRead() && !AloneMyString.newLineOnInput(input))
        {
            line += input;
            input = readS();
        }
    ...
    {
        return AloneMyString.removePattern(" *", readLine(text));
    }
    ...
    public static String readLineWithRegexAndRemoveAllWhiteSpaces(String text, String regex)
    {
        ...
        if (AloneArray.exists(line))
        {
            line = AloneMyString.removeAllWhiteSpaces(line);
        }
    ...
    public static String[] readLinesWithRegex(String... config)
    {
        ...
        if (AloneArray.exists((Object) config) && config.length > 0 && config.length % 2 == 0)
        {
            lines = new String[config.length / 2];
            int counterOfLines = 0;
            String auxLine = readLineWithRegex(config[counterOfLines], config[counterOfLines + 1]);
            
            while (AloneArray.exists(auxLine) && counterOfLines < config.length - 1)
            {
    ...
    public static String[] readLinesWithRegexAndRemoveAllWhiteSpaces(String... config)
    {
        ...
        if (AloneArray.exists((Object) lines))
        {
            for (int i = 0; i < lines.length; i++)
            {
                lines[i] = AloneMyString.removeAllWhiteSpaces(lines[i]);
            }
        }
    ...
    public static float readfloat(String text)
    {
        ...
        return !AloneMyString.hasOneOrNoneDot(data) ? 0 : Float.parseFloat(data);
    }

    public static Float readFloat(String text)
    {
        String data = readSpecificString(text, 'f');
        return !AloneMyString.hasOneOrNoneDot(data) ? null : Float.valueOf(data);
    }

    public static double readdouble(String text)
    {
        String data = readSpecificString(text, 'f');
        return !AloneMyString.hasOneOrNoneDot(data) ? 0 : Double.parseDouble(data);
    }

    public static Double readDouble(String text)
    {
        String data = readSpecificString(text, 'f');
        return !AloneMyString.hasOneOrNoneDot(data) ? null : Double.valueOf(data);
    }
}